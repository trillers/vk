...
Bot.prototype._init = function(){
    ...
    self._listenAgentStatusChange(self.id);
};

/**
 *  options: {
 *      mode: 'untrusted' | 'trusted'
 *      nickname: string
 *      sex: 0 1 2
 *      region: country + space + city
 *  }
 *  start bot
 */
Bot.prototype.start = function(opts){
    ...
    if(opts.mode === 'untrusted'){
        if(!opts.nickname){
            console.warn('bot api[start] input[nickname] error: nickname needed');
        }else{
            msg['Nickname'] = opts.nickname;
        }
        if(!opts.sex && [0, 1, 2].indexOf(opts.sex)){
            console.warn('bot api[start] input[sex] error: sex only accept 0, 1, 2');
        }else{
            msg['Sex'] = opts.sex;
        }
    }
...
Bot.prototype._listenAgentStatusChange = function(id){
    var conf = messageDefinitions.client.clientAgentStatusChange;
...
Bot.prototype.offAgentStatusChange = function(){
    this.removeAllListeners(messageDefinitions.client.clientAgentStatusChange.eventName + this.id);
};

Bot.prototype.onAgentStatusChange = function(handler) {
    this.on(messageDefinitions.client.clientAgentStatusChange.eventName + this.id, handler);
};
